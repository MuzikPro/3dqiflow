    ...
    <>
      ...
      >
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
          ...
          <button style={toggleButtonStyle(false)} onClick={onResetView} title="回到默认机位与注视点">
            复位
          </button>
        </div>
        ...
        <div style={{ fontSize: '9px', color: UI.textFaint, lineHeight: 1.5 }}>
          选「平移」后直接拖动人体到画面中央；按住 Shift 可临时平移。滚轮缩放，复位回正。
        </div>
        <div style={{ fontSize: '11px', color: UI.textMuted }}>流注速度 · {speed.toFixed(1)}x</div>
        <input
          type="range"
          min={0.5}
          max={3}
        ...
        <div style={{ fontSize: '11px', color: UI.textMuted, marginTop: '4px' }}>脏腑位</div>
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
          <button
            style={toggleButtonStyle(anatomicalNodes)}
            onClick={() => anatomicalNodes || onAnatomicalNodesToggle()}
            title="NIH Visible Human 实测形心"
          >
            解剖位
          </button>
          <button
        ...
        <div style={{ fontSize: '9px', color: UI.textFaint, lineHeight: 1.5 }}>
          {anatomicalNodes
            ? '肝胆在右、肾在下肋——解剖真实位，与左升右降的示意布局不同'
            : '肝生于左等圆运动示意布局，非解剖真实位'}
        </div>
        <div style={{ fontSize: '11px', color: UI.textMuted, marginTop: '4px' }}>升降口诀</div>
        <div style={{ fontSize: '12px', lineHeight: 1.7 }}>
          <div style={{ color: COLORS.metal.primary }}>{MNEMONIC.descend}</div>
          <div style={{ color: COLORS.wood.primary }}>{MNEMONIC.ascend}</div>
        </div>
        ...
        <div style={{ fontSize: '9px', color: UI.textFaint, lineHeight: 1.5, marginTop: '2px' }}>
          人体模型：NIH 3D（Visible Human, NLM）· CC BY 4.0 · 详见「声明」页
        </div>
      </div>
      )}
          ...
          return (
            ...
            >
              ...
              <button
                ...
                title={isVisible ? '隐藏此经' : '显示此经'}
                aria-label={`${m.name}显隐`}
                style={{
      ...
      {card && (
        ...
        >
          ...
          <div style={{ fontSize: '12px', color: UI.textMuted, margin: '6px 0 10px' }}>
            {card.path} · {card.direction === 'ascend' ? '↑ 升（左路）' : '↓ 降（右路）'} ·{' '}
            {card.yin ? '阴经（脏）' : '阳经（腑）'}
          </div>
          <div style={{ fontSize: '12px', lineHeight: 1.8, color: UI.textPrimary }}>
            <strong style={{ color: UI.accent }}>升降失常：</strong>
            {card.symptomsIfAbnormal.join('、')}
          </div>
          {(() => {
            ...
            return refs.length > 0 ? (
              <div style={{ fontSize: '11px', color: UI.textSecondary, lineHeight: 1.8, marginTop: '6px' }}>
                <strong style={{ color: UI.accent }}>对应条文：</strong>
                {refs.map((a) => articleLabel(a)).join('、')}（见"条文阅读"）
              </div>
            ) : null;
          })()}
          <Disclaimer />
